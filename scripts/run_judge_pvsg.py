#!/usr/bin/env python3
"""Run Gemini LLM judge evaluation for PVSG dataset (RQ3 re-evaluation).

Evaluates patches from multi-frame video LatentLens results for 2 VideoLLMs
(Molmo2-8B, Qwen2.5-VL-7B) on 100 PVSG videos (50 Ego4D + 50 VidOR).

Usage:
    python scripts/run_judge_pvsg.py \
        --api-key-file gemini_key.txt \
        --layer 24 \
        --output results/judge_evaluation_pvsg.json
"""
...
def get_models_config(n_frames: int) -> dict:
    return {
        "Molmo2-8B": {
            "results_dir": f"results/pvsg_100_molmo2_{n_frames}f_spatial",
        },
        "Qwen2.5-VL-7B": {
            "results_dir": f"results/pvsg_100_qwen25vl_{n_frames}f_spatial",
        },
    }
...
def main():
    ...
    parser.add_argument("--n-frames", type=int, default=4,
                        help="Number of frames used in LatentLens run (determines results dir)")
    parser.add_argument("--output", type=str, default=None,
                        help="Output path (default: results/judge_evaluation_pvsg_{n_frames}f.json)")
    parser.add_argument("--resume", action="store_true", default=True)
    args = parser.parse_args()

    if args.output is None:
        args.output = f"results/judge_evaluation_pvsg_{args.n_frames}f.json"
    ...
    MODELS = get_models_config(args.n_frames)
